...
class LearningAgent(Agent):
    ...
    def __init__(self, env, learning=False, epsilon=1.0, alpha=0.5, Q=None):
    ...
    def choose_action(self, state):
        ...
        if not self.learning:
            ...
            if  self.epsilon > 0.01  and self.epsilon > random.random():
                ...
                #choosing action with maximumQ in current state 
                valid_actions = []
...
def run():
    ...
    try:
		with open('q_data.txt', 'r') as q_file:
			q = ast.literal_eval(q_file.read().strip())
    except:
        q = dict()
    ##############
    ...
    agent = env.create_agent(LearningAgent, learning=True, alpha=0.25, Q=q)
    ...
    sim = Simulator(env,update_delay=0.02, display=True,log_metrics=True)  
    ...
    q = str(agent.Q)
    with open('q_data.txt', 'w') as q_file:
    	q_file.write(q)
